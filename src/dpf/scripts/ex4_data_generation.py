...
solver_names = {
    lightsim2grid.SolverType.GaussSeidel: "GS",
...
def print_configuration():
    ...
    try:
        ...
        tmp = (
            f"- OS: {distro.linux_distribution(full_distribution_name=False)[0]} "
...
def make_grid2op_env(pp_case, case_name, load_p, load_q, gen_p, sgen_p):
    ...
    with warnings.catch_warnings():
        ...
        env_lightsim = make(
            ...
            data_feeding_kwargs={"load_p": load_p, "load_q": load_q, "prod_p": gen_p},
...
def get_loads_gens(load_p_init, load_q_init, gen_p_init, sgen_p_init, prng):
    ...
    load_p = load_p_smooth * prng.lognormal(
        mean=0.0, sigma=0.003, size=load_p_smooth.shape
    )
    load_q = load_q_smooth * prng.lognormal(
        mean=0.0, sigma=0.003, size=load_q_smooth.shape
    )
    if DEBUG:
    ...
    gen_p = (
        load_p.sum(axis=1).reshape(-1, 1)
        / load_p_init.sum()
        * gen_p_init.reshape(1, -1)
    )
    sgen_p = (
        load_p.sum(axis=1).reshape(-1, 1)
        / load_p_init.sum()
...
def run_grid2op_env(
    ...
    if store_observations:
        ...
        if store_as_sparse:
            ...
            data["YBus"] = np.zeros(
                (nb_steps, n_bus_bars, n_bus_bars), dtype=np.complex128
    ...
    if nb_step != nb_ts:
        ...
        total_time = (
            env_lightsim.backend._timer_preproc + env_lightsim.backend._timer_solver
        )  # + env_lightsim.backend._timer_postproc
...
def main():
    ...
    for case_name in tqdm(case_names):
        ...
        # simulate the data
        load_p, load_q, gen_p, sgen_p = get_loads_gens(
        ...
        env_lightsim = make_grid2op_env(
            case, case_name, load_p, load_q, gen_p_g2op, sgen_p
        )
