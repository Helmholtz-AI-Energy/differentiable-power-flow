...
def run_experiment(batchsize, ybus_scaling_method="block_diagonal", density=0.5, max_iter=6):
    sample = 2
    ...
    solver_runtime = solver.run_pf_batched(batchsize, ybus_scaling_method=ybus_scaling_method, density=density,
    ...
    with open(f"out/temp/ex6a_{batchsize}_{ybus_scaling_method}_{density}.pkl", "wb") as writeTOFile:
...
def main():
    ...
    for batch_size in batch_sizes:
        run_experiment(batchsize=batch_size, ybus_scaling_method=ybus_scaling_method, density=density,
                       max_iter=max_iter)
