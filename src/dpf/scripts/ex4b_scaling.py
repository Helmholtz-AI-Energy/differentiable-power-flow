...
def run_experiment(
    ...
    if strategy == "DC":
        ...
        if save_result:
            with open(
                f"out/temp/ex4b_DC_{case_name}_{use_gpu}.pkl", "wb"
            ) as writeDCFile:
    ...
    if strategy == "NR":
        ...
        if save_result:
            with open(
                f"out/temp/ex4b_NR_{case_name}_{use_gpu}.pkl", "wb"
            ) as writeNRFile:
    ...
    if strategy == "TO":
        ...
        if save_result:
            with open(
                f"out/temp/ex4b_TO_{to_optimizer}_{case_name}_{use_gpu}.pkl", "wb"
            ) as writeTOFile:
    ...
    for method in methods:
        for case_name in case_names:
            if method == "TO":
                ...
                run_experiment(
                    method, sample, "None", case_name, use_gpu=use_gpu, save_result=True
                )
