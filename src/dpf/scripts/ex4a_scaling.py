...
def run_experiment(
    ...
    if strategy == "TO":
        ...
        if save_result:
            with open(
                f"out/temp/ex4_TO_{to_optimizer}_{case_name}.pkl", "wb"
            ) as writeTOFile:
        ...
        if report_metrics:
            ...
            with open(
                f"out/temp/ex4_TO_{to_optimizer}_{case_name}_metrics.pkl", "wb"
            ) as writeTOFile:
                pickle.dump(results, writeTOFile)
...
def main(argv=None):
    ...
    for method in ["DC", "NR", "TO"]:
        for case_name_exp in case_names:
            run_experiment(
                method, sample, to_optimizer, case_name_exp, save_result=False
            )
            run_experiment(
                method, sample, to_optimizer, case_name_exp, save_result=True
            )
            if method == "TO":
                run_experiment(
                    method,
