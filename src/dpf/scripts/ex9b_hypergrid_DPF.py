...
def run_experiment(
    max_iter, batchsize, use_gpu, evaluate_losses, strategy, strategy_amount_param
):
    ...
    losses, times, individual_losses = solver.run_pf_super_grid(
    ...
    with open(
        f"differentiable_powerflow/out/ex9b_{use_gpu}_{max_iter}_{batchsize}_{strategy}_{strategy_amount_param}.pkl",
        "wb",
    ) as writeTOFile:
...
def main():
    ...
    for batch_size in batch_sizes_to_report:
        with open(
            f"differentiable_powerflow/out/ex9b_{use_gpu}_{max_iter}_{batch_size}_{strategy}_{strategy_amount_param}.pkl",
            "rb",
        ) as readFile:
    ...
    # plt.show()
    plt.savefig(
        f"out/temp/ex9b_scalability_{use_gpu}_{strategy}_{strategy_amount_param}.png"
    )
    plt.close()
