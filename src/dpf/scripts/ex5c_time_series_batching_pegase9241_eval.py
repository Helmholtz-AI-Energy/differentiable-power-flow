        ...
        for use_gpu in use_gpus:
            ...
            for batch_size in batch_sizes:
                for max_iter in max_iters:
                    with open(f"out/temp/ex5c_time_series_{use_gpu}_{max_iter}_{batch_size}.pkl", "rb") as readTOFile:
        ...
        for use_gpu in use_gpus:
            for batch_size in batch_sizes:
                for max_iter in [1000]:
                    with open(f"out/temp/ex5c_time_series_{use_gpu}_{max_iter}_{batch_size}.pkl",
                              "rb") as readTOFile:
                        results = pickle.load(readTOFile)
                        losses = results["losses"]
                        individual_losses = results["individual_losses"]
                        ...
                        plt.legend()
                        plt.savefig(f"out/plots/ex5c_time_series_batching_losses_{batch_size}.png")
                        plt.close()
