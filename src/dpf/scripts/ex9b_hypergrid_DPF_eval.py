...
def main():
    ...
    for batch_size in batch_sizes_to_report:
        with open(f"out/temp/ex9b_{use_gpu}_{max_iter}_{batch_size}_{strategy}_{strategy_amount_param}.pkl", "rb") as readFile:
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f"out/plots/ex9b_scalability_{use_gpu}_{strategy}_{strategy_amount_param}.png")
    plt.close()
