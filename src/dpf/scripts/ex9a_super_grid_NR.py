...
def run_experiment(batchsize, max_iter=6, strategy="total_random", strategy_amount_param=0):
    ...
    solver_runtime = solver.run_pf_super_grid(batchsize, max_iteration_=max_iter, strategy=strategy,
    ...
    with (open(f"out/temp/ex9a_{batchsize}_{strategy}_{strategy_amount_param}.pkl", "wb") as
          writeTOFile):
...
def main():
    ...
    for batch_size in batch_sizes:
        run_experiment(batchsize=batch_size, max_iter=max_iter, strategy=strategy, strategy_amount_param=strategy_amount_param)
