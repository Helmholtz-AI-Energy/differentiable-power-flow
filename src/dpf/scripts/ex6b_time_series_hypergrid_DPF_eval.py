...
def main():
    ...
    for batch_size in batch_sizes_to_report:
        with open(f"out/temp/ex6b_{use_gpu}_{max_iter}_{batch_size}.pkl", "rb") as readFile:
    ...
    for (j, iteration) in enumerate(iterations_to_report):  # list of iterations to report
        if j == 0:
            plt.plot(batch_sizes_to_report, [x[iteration] for x in to_times], label="DPF (#iterations)", color="green",
                     marker="^", markersize=3)
        plt.plot(batch_sizes_to_report, [x[iteration] for x in to_times], color="green", marker="^", markersize=3)

    for (j, iteration) in enumerate(iterations_to_report):
        plt.annotate("(" + str(iteration) + ")", (batch_sizes_to_report[-1], to_times[-1][iteration]),
